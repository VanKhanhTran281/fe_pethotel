...
import { deleteRoom, fetchRoom } from "../../../redux/actions";
import { deleteRoomData } from "../../../redux/api";
...
const TableRoomList: React.FC = () => {
    ...
    const [valueUd, setValueUd] = useState<RoomData | null>(null);
    const fetchData = () => {
        dispatch(fetchRoom());
    };
    ...
    const [showFormArray, setShowFormArray] = useState(Array(roomDataArray.length).fill(false));
    ...
    const handleUpdateRoom = (updateRoom: RoomData) => {
        dispatch(fetchRoom());
    };
    const handleEditClick = (record: RoomData) => {
    ...
    const handleDeleteClick = async (room: RoomData) => {
        try {
            await deleteRoomData(room.roomId);
    ...
    return (
        <div>
            <div style={{ display: 'flex', width: '1150px', height: '64px', justifyContent: "space-between" }}>
                <div>
                    <p style={{ fontSize: '16px', lineHeight: '19.84px' }}>{roomDataArray?.length} Phòng</p>
                </div>
            ...
            {roomDataArray.length > 0 && (
                <Table style={{ width: '1150px' }} dataSource={filteredRoom} columns={columns} pagination={{ pageSize: 5 }} className="your-table" />
            )}
