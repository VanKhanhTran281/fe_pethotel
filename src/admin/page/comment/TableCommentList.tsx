...
import { deleteComment, deleteService, fetchComment, fetchService } from "../../../redux/actions";
import { deleteCommentData, deleteServiceData } from "../../../redux/api";
import './TableCommentList.css'
import { toast } from "react-toastify";

const TableCommentList: React.FC = () => {
    const dispatch = useAppDispatch();
    const commentData = useSelector((state: RootState) => state.comment.data);
    const commentDataArray = commentData ? Object.values(commentData) : [];
    const [showUpdate, setShowUpdate] = useState(true);
    const [valueUd, setValueUd] = useState<CommentData | null>(null);
    const fetchData = () => {
    ...
    const [showFormArray, setShowFormArray] = useState(Array(commentDataArray.length).fill(false));
    ...
    const handleUpdateComment = (updateComment: CommentData) => {
        dispatch(fetchComment());
    };
    const handleEditClick = (record: CommentData) => {
    ...
    const handleDeleteClick = async (comment: CommentData) => {
        try {
            await deleteCommentData(comment.commentId);
    ...
    const columns = [
        ...
        { title: "Mã phản hồi", dataIndex: "commentId", key: "commentId" },
        { title: "Nội dung", dataIndex: "content", key: "content" },
        {
            title: 'Tùy chọn',
            key: 'actions',
            render: (_: any, record: CommentData, index: number) => (
    ...
    return (
        <div>
            {commentDataArray.length > 0 && (
                <Table style={{ width: '1150px' }} dataSource={commentDataArray} columns={columns} pagination={{ pageSize: 7 }} className="your-table" />
            )}
            {showAlert && (
                <Alert
...
export default TableCommentList