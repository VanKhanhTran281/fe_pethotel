...
import { deleteStaff, deleteUser, fetchStaff, fetchUser } from "../../../redux/actions";
import { deleteStaffData, deleteUserData } from "../../../redux/api";
...
const TableUserList: React.FC = () => {
    ...
    const [valueUd, setValueUd] = useState<UserData | null>(null);
    const fetchData = () => {
        dispatch(fetchUser());
    };
    ...
    const handleUpdateUser = (updateUser: UserData) => {
        dispatch(fetchUser());
    };
    ...
    const columns = [
        ...
        { title: "Mã tài khoản", dataIndex: "userId", key: "userId" },
        ...
        { title: "Vai trò", dataIndex: "role", key: "role" },
        {
            title: 'Tùy chọn',
            key: 'actions',
            render: (_: any, record: UserData, index: number) => (
    ...
    const filteredUser = userDataArray.filter((record) => {
    ...
    return (
        <div>
            <div style={{ display: 'flex', width: '1150px', height: '64px', justifyContent: "space-between" }}>
                <div>
                    <p style={{ fontSize: '16px', lineHeight: '19.84px' }}>{userDataArray.length} Tài khoản</p>
                </div>
                <div style={{ border: '1px solid #E0E4F0 ', marginTop: '7px', display: 'flex', borderRadius: '8px', width: '335px', height: '40px' }}>
                    ...
                    <input style={{ outline: 'none', border: 'none', width: '287px', height: '18px', marginLeft: '5px', marginTop: "13px" }} placeholder="Tìm kiếm bằng tên, email, địa chỉ..." value={searchValue}
                        onChange={(e) => setSearchValue(e.target.value)} />
                </div>
            </div>
            {userDataArray.length > 0 && (
                <Table style={{ width: '1150px' }} dataSource={filteredUser} columns={columns} pagination={{ pageSize: 7 }} className="your-table" />

            )}
            {!showUpdate && <UpdateForm valueUd={valueUd} onUpdate={handleUpdateUser} handleEditClick={handleEditClick} />}
            {showAlert && (
                <Alert
